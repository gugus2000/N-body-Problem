...
for element in temps :
    ...
    for i in range(nb_corps):
          storage[i]={}
    nouvelles_positions = []
    for indexplanete in range(nb_corps) :
        plt.plot(positions_corps[-1][indexplanete]['x'], positions_corps[-1][indexplanete]['y'], ',k')
        somme_force_app = {
            'x' : 0,
            'y' : 0,
            'z' : 0,
        ...
        for i in range(nb_corps) :
            if indexplanete != i :
                if indexplanete not in storage[i]:
                    storage[indexplanete][i] = {}
                    force_x, force_y, force_z = calculForce(positions_corps[-1][indexplanete],positions_corps[-1][i])
                    storage[indexplanete][i]['x'] = force_x
                    storage[indexplanete][i]['y'] = force_y
                    storage[indexplanete][i]['z'] = force_z
                    somme_force_app['x']+=force_x
                    somme_force_app['y']+=force_y
                    somme_force_app['z']+=force_z
                else :
                    somme_force_app['x'] -= storage[i][indexplanete]['x']
                    somme_force_app['y'] -= storage[i][indexplanete]['y']
                    somme_force_app['z'] -= storage[i][indexplanete]['z']
        vitesse = calculVitesse(somme_force_app, positions_corps[-1][indexplanete]['masse'], pas_temps)
        positions_corps[-1][indexplanete]['vitesse']['x'] += vitesse['x']
        positions_corps[-1][indexplanete]['vitesse']['y'] += vitesse['y']
        positions_corps[-1][indexplanete]['vitesse']['z'] += vitesse['z']
        positions_corps[-1][indexplanete]['x'] += positions_corps[-1][indexplanete]['vitesse']['x']*pas_temps
        positions_corps[-1][indexplanete]['y'] += positions_corps[-1][indexplanete]['vitesse']['y']*pas_temps
        positions_corps[-1][indexplanete]['z'] += positions_corps[-1][indexplanete]['vitesse']['z']*pas_temps
        nouvelles_positions.append(positions_corps[-1][indexplanete])
